...
/**
 * Derives 7 HSL tints of a hex color — from lightest (50) to darkest (600).
 * Useful for rendering a shade explorer strip.
 */
export function deriveShades(hex: string): string[] {
  const [h, s] = hexToHsl(hex);
  const stops = [95, 85, 73, 58, 44, 32, 21];
  return stops.map(l => hslToHex(h, Math.min(100, Math.round(s * (l > 50 ? 0.9 : 1))), l));
}
